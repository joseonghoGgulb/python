...
def sevenToTwo(data: list): # convert quaternion data set. it contain 7 float. output data contain 2 float.
    output = []
...
def demensionReduction(data: list): # process of data converting. quaternion data set => featual data set.
    output = []
    for i in data:
        try:
...
def concatData(data1: list, data2: list, data3: list): # concatenate lists. it contain derivatived data
...
def loadDir(path: str, label: list): # read whole data in directory
    ...
    for file_name in file_list:
        ...
        x, y = [], [] # split orignal data by using label tag .  input data is x . ouput data is y.
        for i in data:
            tmpX = []
            for j in i:
                if j == 'label':
